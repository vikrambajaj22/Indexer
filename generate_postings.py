...
# make sorted_postings directory if it doesn't exist
...
# takes about 3hr for 50 WET files i.e. for about 2 million pages/docs (approx. 185 docs per sec)
    ...
    with gzip.open('wet_files/'+filename, 'rt', encoding='utf-8') as f:
        ...
        for line in f:
            if 'WARC/1.0' in line:  # every new page starts with this header information
                ...
                for term in (t for t in ''.join(ch for ch in line.strip() if ch not in string.punctuation).split()):  # generator
                    if all(ord(ch)>32 and ord(ch)<127 for ch in term):  # checking if all characters in the term are ASCII (0 through 31, and 127, represent non-printable control characters; 32 is space)
...
# using Unix merge to merge the 50 posting files into a single file (takes about 25min with buffer-size 1024; final file has ~0.5 billion postings)
os.chdir('sorted_postings')
os.system('bash; sort -m -k1,1 -k2n,2 --buffer-size=1024 *.txt > final_postings.txt; mkdir final_postings')  # uses the new Linux bash in Windows 10, also -m: merge, dont re-sort files
